...
public abstract class AbstractApplicationContext
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			 ...
			 *  准备上下文的刷新工作，记录bean容器的启动时间，容器活跃状态,验证系统中一些属性和属性值的设置等.
			...
			/**
			 * 3. bean工厂初始化前的准备工作，设置bean工厂的一些属性
			...
			try {
				/**
				 * 4. 空方法，子类可以覆盖此方法，添加一些特殊的BeanPostProcessors
				 * 这时Bean加载注册完，但尚未实例化.
				 */
				...
				/**
				 * 9. 典型的模板方法（钩子方法），默认是空方法，未实现，用来初始化一些特殊的Bean。
				 * 具体子类可以通过继承AbstractApplicationContext类，然后覆写该方法进行自定义特殊bean的初始化.
				 *
				...
				/** 12. 发布事件, ApplicationContext初始化完成
				 * 例如容器中的刷新事件:ContextRefreshedEvent就是在这一步中发布. SpringCloud在该步骤中会启动web服务
				 */
				finishRefresh();
			}
	...
	/**
	 * 在应用程序上下文的标准初始化后修改其内部bean工厂。
	 * 这时所有bean定义已经加载，但尚未实例化。
	 * 这允许在某些ApplicationContext实现中注册特殊BeanPostProcessor等
	...
	protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		...
		// 如果存在名为“loadTimeWeaver”的bean，添加
